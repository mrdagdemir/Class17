...
public class Notes {
    ...
    private default(it's not a keyword) protected and public. We have three keywords that
...
Default(its not a keyword):
    ...
    classes which exists inside the same package.
Protected:
    When we have protected access modifier we can access that method or variable inside the same class+ inside
    the other classes which are inside the same package + inside the classes which are in different packages
    as well but they must have extends relationship with the original class.
Public:
    When we use the public access modifiers we can use that field or method anywhere inside
    the same project. we can access them in the class+ in the package+ in other packages +
    in the project.
